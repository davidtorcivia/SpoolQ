...
fn render_rust_sync_slice(syncs: &[SyncStep]) -> String {
    let items = syncs
        .iter()
        .map(|sync| format!("SyncStep::{}", sync.rust_name()))
        .collect::<Vec<_>>();
    let inline = format!("&[{}]", items.join(", "));
    if "        required_syncs: ".len() + inline.len() < 100 {
        return inline;
    }

    let mut multiline = String::from("&[\n");
    for item in items {
        writeln!(multiline, "            {item},").expect("writing to String cannot fail");
    }
    multiline.push_str("        ]");
    multiline
}
...
"#,
    ...
    for transition in &spec.transitions {
        let required_syncs = render_rust_sync_slice(&transition.required_syncs);
    ...
    for exception in &spec.exceptions {
        let required_syncs = render_rust_sync_slice(&exception.required_syncs);
        writeln!(
            output,
