...
- TLC checks `TypeInvariant`, `CompleteVisibleEnvelope`, `LeaseHasToken`, `AttemptWithinLimit`, `ReceiptIsTerminal`, and `DeliveredAttemptIsPositive` separately. Their exact bounded meanings and omissions are documented in `model/README.md`; they are abstract predicates, not production-code equivalence evidence.
